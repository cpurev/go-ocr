...
func run() error {
	...
	outbox := store.Outbox(store.NewMemoryOutbox())
	...
	if cfg.MongoURI != "" {
		...
		relayOutbox := store.NewMongoOutbox(db.Collection(cfg.MongoSeen), db.Collection(cfg.MongoOutbox))
		...
		if err := relayOutbox.EnsureIndexes(indexCtx); err != nil {
			return err
		}
